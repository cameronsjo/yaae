...
import type { DocumentSettings, LinksMode, ThemeMode, FontPreset } from '../settings';
...
export function buildPrintDocumentState(
  ...
  if (vPdf) {
    ...
    if ('fontFamily' in rawPdf && vPdf.fontFamily) state.fontFamily = vPdf.fontFamily;
    if ('fontSize' in rawPdf && typeof vPdf.fontSize === 'number') state.fontSize = vPdf.fontSize;
    if ('lineHeight' in rawPdf && typeof vPdf.lineHeight === 'number') state.lineHeight = vPdf.lineHeight;
    if ('copyPasteSafe' in rawPdf) state.copyPasteSafe = vPdf.copyPasteSafe !== false;
    if ('compactTables' in rawPdf) state.compactTables = vPdf.compactTables !== false;
    if ('pageNumbers' in rawPdf) state.pageNumbers = vPdf.pageNumbers !== false;
    if ('headerLeft' in rawPdf && typeof vPdf.headerLeft === 'string') state.headerLeft = vPdf.headerLeft;
    if ('headerRight' in rawPdf && typeof vPdf.headerRight === 'string') state.headerRight = vPdf.headerRight;
    if ('footerLeft' in rawPdf && typeof vPdf.footerLeft === 'string') state.footerLeft = vPdf.footerLeft;
    if ('footerRight' in rawPdf && typeof vPdf.footerRight === 'string') state.footerRight = vPdf.footerRight;
    if ('watermarkText' in rawPdf && typeof vPdf.watermarkText === 'string' && vPdf.watermarkText) {
    ...
    if ('links' in rawPdf || 'plainLinks' in rawPdf || 'expandLinks' in rawPdf) {
