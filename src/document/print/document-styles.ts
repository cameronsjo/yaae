...
const FONT_PRESETS: Set<FontPreset> = new Set<FontPreset>([
  "sans",
  "serif",
  "mono",
  "system",
...
export function deriveStateClasses(state: PrintDocumentState): string[] {
  ...
  if (state.linksMode !== "expand")
    classes.push(`pdf-links-${state.linksMode}`);
...
export function buildDocumentCss(
  ...
  if (activated)
    sections.push(`/* --- state-activated rules --- */\n${activated}`);
...
@media print {
  ...
  const fontSize = clampNumber(
    ...
    6,
    72,
    DEFAULT_DOCUMENT_SETTINGS.fontSize,
  );
  const lineHeight = clampNumber(
    state.lineHeight,
    1,
    3,
    DEFAULT_DOCUMENT_SETTINGS.lineHeight,
  );
...
${typographyRules.join("\n")}
  ...
  if (state.watermark !== "off") {
    const svgFont = Object.hasOwn(FONT_PRESET_SVG, state.fontFamily)
      ...
      : state.fontFamily;
    const uri = buildWatermarkDataUri(
      state.watermark,
      state.watermarkText,
      svgFont,
      state.watermarkPresets,
    );
    const wp = {
      ...WATERMARK_PRESETS[state.watermark],
      ...(state.watermarkPresets?.[state.watermark] ?? {}),
    };
    const size = wp.tileSize;
