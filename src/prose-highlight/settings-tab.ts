...
export function renderProseHighlightSettings(
  ...
  new Setting(proseContent)
    ...
    .setDesc(
      "Color-code adjectives, nouns, adverbs, verbs, and conjunctions in the editor.",
    )
    .addToggle((toggle) =>
  ...
  new Setting(proseContent)
    ...
    .setDesc(
      "Also apply prose highlighting when viewing notes in Reading View.",
    )
    .addToggle((toggle) =>
      toggle.setValue(settings.readingViewEnabled).onChange(async (value) => {
  ...
  hint.appendText(
    "Toggle which categories highlight. Customize colors via the ",
  );
  ...
  for (const cat of POS_CATEGORIES) {
    ...
    new Setting(posContent).setName(POS_LABELS[cat]).addToggle((toggle) =>
...
function renderCustomWordLists(
  ...
  function redraw() {
    ...
    // Add list button
    new Setting(listContainer).setName("Add word list").addButton((btn) =>
      btn
        .setButtonText("+ New List")
        .setCta()
        .onClick(async () => {
...
function renderWordListEntry(
  ...
  const header = new Setting(wrapper).setName(list.name).addToggle((toggle) =>
