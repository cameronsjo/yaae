...
- **Is the model lazy-loadable? — Not needed. wink starts up faster than compromise.** The `pnpm bench` viewport figures measure a fresh tagger against an already-loaded model, since `winkNLP(model)` runs at module scope; they say nothing about startup. Measured separately (Node, 3 runs): requiring the model costs 17–26 ms and `winkNLP()` init 36–41 ms, **54–67 ms total, against 79–83 ms to require compromise and tag once**. Obsidian parses every enabled plugin's `main.js` at launch, so this is the number that matters there, and it moves the right way. Lazy-loading is a complication with nothing to buy; drop it from the swap design.
- **Mobile.** Prose highlighting is currently disabled on mobile (yaae#32). Mobile users would carry the model's download weight for a feature they cannot use. The startup measurement above says they would not pay a *time* cost for it, so this is a download-size question only.
