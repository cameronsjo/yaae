...
const DOC_WITHOUT_TOC = ['# Title', '', '## Section One', '', 'Body text.'].join('\n');
...
function makeFakeVault(opts: { enabled?: () => boolean; depth?: number } = {}): FakeVault {
...
describe('auto TOC wiring in main.ts', () => {
  ...
  it('resolveDepth routes through the shared resolveTocDepth helper', () => {
    ...
    const tocFn = MAIN_TS.match(/async\s+generateTocForCurrentFile[\s\S]*?\n\s{2}\}/);
