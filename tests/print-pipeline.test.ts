...
function makeState(
  overrides: Partial<PrintDocumentState> = {},
): PrintDocumentState {
...
describe("resolvePrintVars", () => {
  ...
  it("covers every declared knob", () => {
    ...
    expect(Object.keys(vars).sort()).toEqual(
      Object.keys(PRINT_VAR_DEFAULTS).sort(),
    );
  });
  ...
  it("drops the placebo page-number position knobs", () => {
    expect(PRINT_VAR_DEFAULTS).not.toHaveProperty(
      "--yaae-print-page-number-bottom",
    );
    expect(PRINT_VAR_DEFAULTS).not.toHaveProperty(
      "--yaae-print-page-number-right",
    );
  });
});
...
describe("bakePrintVars", () => {
  it("replaces bare var() references", () => {
    expect(
      bakePrintVars("a { color: var(--yaae-print-link-url-color); }", VARS),
    ).toBe("a { color: #666; }");
  });
  ...
  it("replaces two-arg var(--x, fallback) references", () => {
    expect(
      bakePrintVars(
        "x { margin-top: var(--yaae-print-sig-margin-top, 3em); }",
        VARS,
      ),
  ...
  it("uses the inline fallback for unknown knobs", () => {
    expect(
      bakePrintVars("x { top: var(--yaae-print-unknown, 5px); }", VARS),
    ).toBe("x { top: 5px; }");
  });
...
describe("buildMarginBoxCss", () => {
  ...
  it("generates @top-center for built-in classification", () => {
    const css = buildMarginBoxCss(
      makeState({ classification: "confidential" }),
      VARS,
    );
  ...
  it("generates @bottom-center when bannerPosition is both", () => {
    const css = buildMarginBoxCss(
      makeState({ classification: "internal", bannerPosition: "both" }),
      VARS,
    );
    expect(css).toContain("@top-center");
  ...
  it("omits @bottom-center when bannerPosition is top", () => {
    const css = buildMarginBoxCss(
      makeState({ classification: "internal", bannerPosition: "top" }),
      VARS,
    );
    expect(css).toContain("@top-center");
  ...
  it("omits @bottom-center when signatureBlock is true", () => {
    const css = buildMarginBoxCss(
      makeState({ classification: "confidential", signatureBlock: true }),
      VARS,
    );
    expect(css).toContain("@top-center");
    expect(css).not.toContain("@bottom-center");
  ...
  it("resolves custom classifications", () => {
    ...
    const css = buildMarginBoxCss(
      makeState({ classification: "secret", customClassifications: customs }),
      VARS,
    );
  ...
  it("places headers, footers, and page numbers in their margin boxes", () => {
    const css = buildMarginBoxCss(
      makeState({
        ...
        headerRight: "Engineering",
        footerLeft: "SENSITIVE",
        footerRight: "v2.0",
        pageNumbers: true,
      }),
      VARS,
  ...
  it("styles chrome text from resolved knobs, not hardcoded values", () => {
    ...
    const css = buildMarginBoxCss(
      makeState({ classification: "internal", headerLeft: "Acme" }),
      vars,
    );
  ...
  it("sanitizes classification colors", () => {
    ...
    const css = buildMarginBoxCss(
      makeState({ classification: "bad", customClassifications: customs }),
      VARS,
    );
  ...
  it("omits the banner for an unrecognized classification", () => {
    expect(
      buildMarginBoxCss(makeState({ classification: "nonexistent" }), VARS),
    ).toBe("");
  });

  it("auto theme nests prefers-color-scheme inside @media print", () => {
    const css = buildMarginBoxCss(
      makeState({ classification: "confidential", theme: "auto" }),
      VARS,
    );
  ...
  it("light theme emits no prefers-color-scheme override", () => {
    const css = buildMarginBoxCss(
      makeState({ classification: "confidential", theme: "light" }),
      VARS,
    );
...
describe("buildFixedChromeCss", () => {
  ...
  it("keeps the universal @page margin", () => {
    const css = buildFixedChromeCss(
      makeState({ classification: "internal" }),
      VARS,
    );
    expect(css).toContain("margin: 1in");
...
describe("buildDocumentCss", () => {
  ...
  it("renders the active watermark on the content box on Chrome < 131", () => {
    ...
    expect(css).toContain(
      ".print .markdown-preview-view.markdown-preview-view",
    );
  });

  it("emits exactly one watermark layer for the active level (no double-dose)", () => {
    const overlayCss = buildDocumentCss(
      makeState({ watermark: "loud" }),
      VARS,
      true,
    );
    const contentCss = buildDocumentCss(
      makeState({ watermark: "loud" }),
      VARS,
      false,
    );
  ...
  it("does not sync a pdf-watermark-* body class (would double the layer)", () => {
    expect(deriveStateClasses(makeState({ watermark: "loud" }))).not.toContain(
      "pdf-watermark-loud",
    );
  });
  ...
  it("embeds the watermark text in the SVG data URI", () => {
    const css = buildDocumentCss(
      makeState({ watermark: "loud", watermarkText: "SECRET" }),
      VARS,
      true,
    );
  ...
  it("emits font-size rule only when off-default", () => {
    expect(buildDocumentCss(makeState(), VARS)).not.toContain(
      "font-size: 11pt",
    );
    expect(buildDocumentCss(makeState({ fontSize: 14 }), VARS)).toContain(
      "font-size: 14pt",
    );
  });
...
describe("activateStateCss", () => {
  ...
  it("rewrites active class rules to view-targeted selectors", () => {
    const out = activateStateCss(
      css,
      new Set(["pdf-copy-safe", "pdf-links-plain"]),
    );
...
describe("buildPrintDocumentState", () => {
  ...
  it("explicit draft status engages the default draft watermark", () => {
    ...
    expect(state.watermark).toBe(
      DEFAULT_DOCUMENT_SETTINGS.defaultWatermarkForDrafts,
    );
  });
...
describe("buildWatermarkDataUri", () => {
  ...
  it("escapes XML special characters in text and font", () => {
    const uri = decodeURIComponent(
      buildWatermarkDataUri("loud", `<&>"'`, `Font's <X>`),
    );
  ...
  it("accepts per-level overrides that win over defaults", () => {
    const uri = decodeURIComponent(
      buildWatermarkDataUri("loud", "X", "sans-serif", {
        loud: { opacity: 0.99, fontSize: 200, tileSize: 100, rotation: 0 },
      }),
    );
    expect(uri).toContain("rgba(0,0,0,0.99)");
    expect(uri).toContain("font-size='200'");
    expect(uri).toContain("width='100'");
    expect(uri).toContain("rotate(0,");
  });

  it("partial overrides merge with defaults (only opacity changed)", () => {
    const uri = decodeURIComponent(
      buildWatermarkDataUri("loud", "X", "sans-serif", {
        loud: { opacity: 0.5 },
      }),
    );
    expect(uri).toContain("rgba(0,0,0,0.5)");
    // fontSize, tileSize, rotation unchanged from defaults
    expect(uri).toContain("font-size='100'");
    expect(uri).toContain("rotate(-35,");
  });

  it("override for one level does not affect others", () => {
    const whisperUri = decodeURIComponent(
      buildWatermarkDataUri("whisper", "X", "sans-serif", {
        loud: { opacity: 0.99 },
      }),
    );
    expect(whisperUri).toContain("rgba(0,0,0,0.03)");
  });

  it("handles null overrides gracefully", () => {
    const uri = decodeURIComponent(
      buildWatermarkDataUri("loud", "X", "sans-serif", undefined),
    );
    expect(uri).toContain("rgba(0,0,0,0.14)");
  });
